...
class SellerLogic
{
    public function login($username, $password)
    {
        ...
        $admin = Db::name('seller')->alias('a')->join('__ADMIN_ROLE__ ar', 'a.role_id=ar.role_id')->where($condition)->find();
        if (!$admin) {
    ...
    {
        Db::name('seller')->where('seller_id', $admin['seller_id'])->save([
            'last_login' => time(),
            'last_ip' => request()->ip()
        ]);
        ...
        session('seller_id', $admin['seller_id']);
